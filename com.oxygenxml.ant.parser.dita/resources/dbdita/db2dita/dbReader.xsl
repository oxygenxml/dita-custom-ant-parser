...
<xsl:stylesheet version="2.0" 
				...
				xmlns:xl="http://www.w3.org/1999/xlink" exclude-result-prefixes="xl">
...
<xsl:template match="*[@xml:id]" mode="id.att.in">
  <xsl:param name="isRequired"/>
...
<xsl:template match="*[not(@xml:id)]" mode="topic.topic.id.att.in">
  <xsl:param name="isRequired"/>
  <xsl:variable name="value">
...
<xsl:template match="*" mode="topic.body.content.in">
  <xsl:apply-templates select="ackno|address|anchor|annotation|
            ...
            table|tip|variablelist|warning|note"
...
<xsl:template match="step" mode="task.step.child">
  <xsl:variable name="firstInfo" select="*[
        not(local-name() = 'para' and count(preceding-sibling::*) = 0) and 
...
<xsl:template match="step" mode="task.cmd.content.in">
  <xsl:apply-templates select="*[local-name() = 'para'][count(preceding-sibling::*) = 0] | text()"></xsl:apply-templates>
</xsl:template>
...
<xsl:template match="biblioref|link|xref" mode="title.cnt.child">
  <xsl:apply-templates mode="textOnly"/>
</xsl:template>

<xsl:template match="biblioref|link|xref" mode="child">
  <xsl:apply-templates select="." mode="topic.xref.out"/>
</xsl:template>
...
<xsl:template match="link[@xl:href]" mode="href.att.in">
  <xsl:param name="isRequired"/>
  <xsl:apply-templates select="." mode="href.att.out">
    <xsl:with-param name="value" select="@xl:href"/>
  </xsl:apply-templates>
</xsl:template>

<xsl:template match="link" mode="format.att.in">
  <xsl:if test="contains(@xl:href, 'http://') or contains(@xl:href, 'https://') or contains(@xl:href, 'www.') or contains(@xl:href, '.html') or contains(@xl:href, '.htm') or contains(@xl:href, '.asp') or contains(@xl:href, '.aspx')">
    <xsl:apply-templates select="." mode="format.att.out">
      <xsl:with-param name="value" select="'html'"/>      
    </xsl:apply-templates>
  </xsl:if>
</xsl:template>
  
<xsl:template match="link" mode="scope.att.in">
  <xsl:if test="contains(@xl:href, 'http://') or contains(@xl:href, 'https://') or contains(@xl:href, 'www.') or contains(@xl:href, '.html') or contains(@xl:href, '.htm') or contains(@xl:href, '.asp') or contains(@xl:href, '.aspx')">
    <xsl:apply-templates select="." mode="scope.att.out">
      <xsl:with-param name="value" select="'external'"/>      
    </xsl:apply-templates>
  </xsl:if>
</xsl:template>  

<xsl:template match="*[biblioref or link or xref]"
      mode="topic.xref.in">
  <xsl:param name="container"/>
  <xsl:param name="isRequired"/>
  <xsl:apply-templates select="biblioreflink|link|xref"
        mode="topic.xref.out"/>
</xsl:template>
...
<xsl:template match="co[@xml:id]" mode="child">
  <xsl:apply-templates select="." mode="topic.keyword.out"/>
</xsl:template>

<xsl:template match="co[@xml:id and @label]" mode="topic.keyword.content.in">
  <xsl:value-of select="@label"/>
</xsl:template>

<xsl:template match="co[@xml:id and not(@label)]" mode="topic.keyword.content.in">
...
<xsl:template match="callout" mode="listitem.cnt.text.in">
  <xsl:param name="container"/>
  <xsl:param name="isRequired" select="'no'"/>
  <xsl:variable name="called"  select="id(@arearefs)[1]"/>
...
<xsl:template match="graphic[@fileref]|imageobject[imagedata/@fileref]|
  ...
  <xsl:template match="graphic[@entityref]|imageobject[imagedata/@entityref]|
    inlinegraphic[@entityref]|
    mediaobject[imageobject/imagedata/@entityref]|
    inlinemediaobject[imageobject/imagedata/@entityref]"
    mode="child">
    <xsl:apply-templates select="." mode="topic.image.out"/>
  </xsl:template>
  

<xsl:template match="graphic[not(@fileref) and not(@entityref)]|
  imageobject[not(imagedata/@fileref) and not(imagedata/@entityref)]|
            inlinegraphic[not(@fileref) and not(@entityref)]|
            mediaobject[not(imageobject/imagedata/@fileref) and not(imageobject/imagedata/@entityref)]|
            inlinemediaobject[not(imageobject/imagedata/@fileref) and not(imageobject/imagedata/@entityref)]"
      mode="child">
...
<xsl:template
  ...
  <xsl:template
    match="graphic[@entityref]|imagedata[@entityref]|inlinegraphic[@entityref]"
    mode="href.att.in">
    <xsl:param name="isRequired"/>
    <xsl:apply-templates select="." mode="href.att.out">
...
<xsl:template name="makeRef">
  ...
  <xsl:otherwise>
    <xsl:apply-templates select="$target" mode="keyref.att.out">
...
<xsl:template match="*[@xml:id]" mode="targetId" priority="100">
