...
<!--= = = ELEMENT OUTPUT RULES  = = = = = = = = = =-->
   ...
   <xsl:template match="*" mode="topic.section.out">
      <section>
         ...
         <xsl:apply-templates select="." mode="topic.section.content.in"/>
      </section>
   </xsl:template>
   ...
   <xsl:template match="*" mode="topic.tgroup.out">
      <tgroup>
         ...
         <xsl:if test="not(@cols)">
            <xsl:attribute name="cols"><xsl:value-of select="count(*:tbody/*:tr[1]/*:td)"/></xsl:attribute>
         </xsl:if>
...
<!--= = = ATTRIBUTE OUTPUT RULES  = = = = = = = = =-->
   ...
   <xsl:template match="*" mode="frame.att.out">
      ...
      <xsl:attribute name="frame">
         <xsl:choose>
            <xsl:when test="$value = 'border'">all</xsl:when>
